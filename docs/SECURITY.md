...
- `orama-node` runs as the `orama` user. The root actions it needs — managing Orama's own systemd units and opening the TURN ports in UFW — go through one binary, `/usr/local/bin/orama-privhelper`, which `/etc/sudoers.d/orama-namespaces` grants with no argument pattern: `orama ALL=(root) NOPASSWD: /usr/local/bin/orama-privhelper`
- The helper parses its own argv (`pkg/privhelper`) and refuses anything else before starting a process: `systemctl start|stop|restart|enable|disable` on `orama-namespace-<svc>@<ns>`, `orama-deploy-<runtime>@<instance>.service` and `orama-turn.service`; `stop|disable` on the pre-namespace units the index migration retires; `daemon-reload`; `set-property` of `MemoryMax`/`CPUQuota` on deployment units; `ufw status|reload`, and `ufw [delete] allow` of the TURN listeners (3478/udp, 3478/tcp, 5349/tcp) and UDP relay ranges inside 49152-65535. It runs `systemctl`/`ufw` from fixed paths with a fixed `PATH`
- This replaced sudoers rules with wildcard arguments (`systemctl start orama-namespace-*`, `ufw allow *`). sudo-rs — the default sudo on Ubuntu 26.04 — refuses wildcards in arguments, so those rules did not load there; and in classic sudo `*` matches spaces and further arguments, so `ufw allow *` granted any firewall rule, including opening internal ports such as rqlite's
- Read-only queries (`systemctl is-active`) need no privilege and never go through the helper
- The gateway's unit sets `NoNewPrivileges=yes`, under which sudo cannot gain root, so the gateway itself has no path to the helper; `orama-node`'s unit deliberately omits it for that reason
