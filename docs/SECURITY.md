...
- Resource selectors (`storage:avatars/*`, `pubsub:topic=chat.*`) are validated and stored, and **not enforced**: the data plane checks a grant and has no notion of which object within it is being touched. A grant carrying a selector therefore authorises **nothing**. Handing over the whole role instead would turn "may write to storage:avatars/\*" into "may write to all storage" — a narrower-looking grant that is in fact the wide one. The API and the CLI say so on every such grant
