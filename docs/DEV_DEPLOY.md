...
#### Privileged helper replaces the wildcard sudoers rules (first upgrade to this release)

`orama-node` runs as the `orama` user; its root actions (Orama's own systemd units, the TURN firewall rules) now go through `/usr/local/bin/orama-privhelper`, granted in `/etc/sudoers.d/orama-namespaces` with no argument pattern — see [SECURITY.md](SECURITY.md). The old rules used wildcard arguments, which sudo-rs (the default sudo on Ubuntu 26.04) refuses. On the first upgrade to this release everything before the re-exec is the old binary, which writes the old rules and does not install the helper, so the post-swap steps under the **new** binary install the helper and then the grant, before Phase 4b and before `orama-node` restarts. A release archive without `bin/orama-privhelper` fails the upgrade at that step with nothing restarted.
