...
| Resource | Minimum (enforced by installer) | Recommended |
|----------|---------|-------------|
| CPU | 2 vCPUs | 4 vCPUs (dedicated) |
| RAM | 2 GB | 8 GB |
| Storage | 10 GB free | 160 GB NVMe |
...
All other inter-node communication travels over the WireGuard overlay (`10.0.0.x` subnet) and does not require public port exposure. The Tor client's SOCKS5 proxy binds to `127.0.0.1:9050` and is never exposed publicly.
...
Every Orama node installs a [Tor](https://www.torproject.org) client for anonymized outbound requests (`/v1/proxy/anon`, `/v1/proxy/tunnel` and the `anon_fetch` function host call). It is client only — no relay, no exit, no inbound port — and there is no flag for it: `orama node install` and `orama node upgrade` always set it up.

- Installed from the Tor Project's repository (`deb.torproject.org`), with the archive key pinned by fingerprint. The OS must be one the Tor Project publishes for: Ubuntu 22.04 (`jammy`), 24.04 (`noble`), 26.04 (`resolute`), Debian 12 (`bookworm`) or 13 (`trixie`).
- Runs as `orama-namespace-tor@index` (user `debian-tor`), with its config in `/etc/orama/tor/torrc` and its state in `/var/lib/orama-tor`. The package's own `tor.service` is masked.
- The SOCKS5 proxy binds to `127.0.0.1:9050` and is never reachable off-host.

Nodes installed before Tor replaced the Anyone network have Anyone removed on their next upgrade — units, the `anon` package, its apt source and all of `/etc/anon`, `/var/lib/anon` and `/var/log/anon`. The old `--anyone-*` flags now fail with an error explaining that Tor is installed automatically.
...
The upgrade command also accepts `--env`, `--node`, and `--delay` for targeting specific environments and nodes. Every upgrade also upgrades the Tor client from `deb.torproject.org` before the node's services stop, and on a node that still has it removes the Anyone network.
