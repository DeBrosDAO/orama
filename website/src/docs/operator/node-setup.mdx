...
```bash
orama node logs node       # Node service logs (includes RQLite)
orama node logs gateway    # Gateway logs (embedded in the node service)
orama node logs ipfs       # IPFS daemon logs
orama node logs cluster    # IPFS Cluster logs
orama node logs olric      # Olric cache logs
orama node logs rqlite     # RQLite logs (included in node service logs)
```
