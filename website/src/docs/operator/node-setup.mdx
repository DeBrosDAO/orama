...
All other inter-node communication travels over the WireGuard overlay (`10.0.0.x` subnet) and does not require public port exposure. The Anyone client's SOCKS5 proxy binds to `127.0.0.1:9050` and is never exposed publicly.
...
Every Orama node installs an [Anyone Protocol](https://anyone.io) client for anonymized outbound requests (`/v1/proxy/anon`). Relay operator mode is not supported — nodes only run as clients.

### Example: Installing with the Anyone Client
...
| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--anyone-client` | bool | `false` | Install Anyone as a local SOCKS5 proxy (port 9050, localhost only) |
...
The upgrade command also accepts `--anyone-client` to reinstall the Anyone client during an upgrade, plus `--env`, `--node`, and `--delay` for targeting specific environments and nodes.
