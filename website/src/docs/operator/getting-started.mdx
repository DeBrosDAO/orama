...
`orama node setup` only accepts `--anyone-relay`. The finer-grained Anyone flags below belong to `orama node install` (and `orama node upgrade`), which run directly on the VPS:
