...
The build pipeline:
1. Cross-compiles all Go binaries for `linux/<arch>` (CLI, gateway, node, SFU, TURN, identity, SNI router)
...
```
devnet → testnet → sandbox
```

- **devnet** — First deployment target, used for active development and testing
- **testnet** — Staging environment, mirrors sandbox configuration
- **sandbox** — Production-facing cluster (`dbrs.space`), requires extra verification and approval. This is the CLI's default active environment.

There is no separate `production` environment name in the CLI — `dbrs.space` is configured as `sandbox` (defaults: `sandbox`, `devnet`, `testnet`).
...
| Stage | What It Does |
|-------|-------------|
| Go tests | `go vet ./...` and `go test -race ./...`, run in `core/` |
| SDK build | `pnpm typecheck`, `pnpm build`, `pnpm vitest run tests/unit` in `sdk/` |
| Version sanity | Checks the root `VERSION` file matches `sdk/package.json` |

There is no `go fmt` check, no all-binaries build stage, and no E2E stage in CI — E2E tests are run manually against a sandbox or real environment.
