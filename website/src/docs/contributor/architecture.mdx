...
Privacy client integration on every node:

- **Client mode** — SOCKS5 proxy for anonymized outbound requests (`/v1/proxy/anon`), bound to `127.0.0.1:9050`
- **Relay operator mode is not supported** — every node runs an Anyone client only
...
| Anyone | 9050 | Localhost only (`127.0.0.1`) |
