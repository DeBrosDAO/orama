...
Anonymity client on every node:

- **Client only** — `orama-namespace-tor@index`, a SOCKS5 proxy for anonymized outbound requests (`/v1/proxy/anon`, `/v1/proxy/tunnel`, the `anon_fetch` host function), bound to `127.0.0.1:9050`
- **No relay, no exit** — the torrc sets `ClientOnly 1`, `ORPort 0`, `DirPort 0`, `ExitRelay 0`; nothing is opened in the firewall for Tor
...
| Tor (SOCKS5) | 9050 | Localhost only (`127.0.0.1`) |
...
**OramaOS** is in development; today devnet and testnet nodes run Ubuntu with systemd hardening. OramaOS is a custom minimal Linux image built with Buildroot:
