...
`core/cmd/` contains entry points for these binaries. `make build` and the `orama build` deployment archive compile all of them except `orama-inspector`, which currently has no build target in the Makefile or builder:
...
| `orama-inspector` | `cmd/inspector/` | SSH-based health inspector (not built by `make build`; run via `go run ./cmd/inspector`) |
