...
| **Proxy** | Anonymous HTTP proxying through the Tor network |
...
| Tor | Anonymity network for the proxy (client only on every node) |
...
|-------------|-------------|
| `sandbox` | `https://dbrs.space` |
| `devnet` | `https://orama-devnet.network` |
| `testnet` | `https://orama-testnet.network` |

The default active environment is `sandbox`. You can override the gateway URL for any command by setting the `ORAMA_GATEWAY_URL` environment variable.
