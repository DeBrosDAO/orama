...
Publish string or binary payloads to any topic. Messages are base64-encoded on
the wire. Every delivered message carries both `bytes`, the payload exactly as
published, and `data`, the same payload decoded as UTF-8 text — use `bytes` for
anything that is not text, because the text view replaces bytes that do not form
valid UTF-8.
...
A subscription opened with a signed-in session is held to that session's token: the gateway closes it with `4403` within 10 seconds of the token being revoked, and with `4401` two minutes after the token expires. Subscribe again with a fresh token to continue.
...
interface PubSubMessage {
  data: string;      // The payload decoded as UTF-8 text
  bytes: Uint8Array; // The payload exactly as it was published
...
UTF-8 are replaced with U+FFFD. Read `bytes` for anything binary.
