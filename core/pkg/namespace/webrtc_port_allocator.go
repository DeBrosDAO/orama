...
// HostHasTURN reports whether the PHYSICAL HOST behind nodeID already runs TURN
// for any namespace.
//
// TURN binds the fixed ports 3478/5349, which are exclusive per host, not per
// node ID — and a host can carry several logical node IDs (see
// getColocatedNodeIDs). NodeHasTURN only asks about one node ID, so two
// colocated IDs both look TURN-free and the second allocation crash-loops on
// "address already in use". This joins through dns_nodes.ip_address so the
// question matches the resource actually being contended.
func (wpa *WebRTCPortAllocator) HostHasTURN(ctx context.Context, nodeID string) (bool, error) {
	internalCtx := client.WithInternalAuth(ctx)
	...
	query := `
		...
		JOIN dns_nodes self ON self.id = ?
		JOIN dns_nodes peer ON peer.id = wpa.node_id
		WHERE wpa.service_type = 'turn' AND peer.ip_address = self.ip_address
	`
	if err := wpa.db.Query(internalCtx, &results, query, nodeID); err != nil {
		return false, &ClusterError{
			Message: "failed to check host-wide TURN allocation",
			Cause:   err,
		}
	}
	if len(results) == 0 {
...
// HostHasTURNForOtherCluster reports whether the physical host behind nodeID
// runs TURN for a namespace OTHER than the given cluster.
//
// Used before spawning a newly-gained TURN role: 3478/5349 are host-exclusive,
// and the previous holder only stops on its own next reconcile tick, so starting
// here during that window would crash-loop on "address already in use". Excluding
// this cluster means a node re-spawning its OWN role is never blocked by itself.
func (wpa *WebRTCPortAllocator) HostHasTURNForOtherCluster(ctx context.Context, nodeID, clusterID string) (bool, error) {
	internalCtx := client.WithInternalAuth(ctx)
	...
	query := `
		...
		WHERE wpa.service_type = 'turn'
		  AND peer.ip_address = self.ip_address
		  AND wpa.namespace_cluster_id != ?
	`
	if err := wpa.db.Query(internalCtx, &results, query, nodeID, clusterID); err != nil {
		return false, &ClusterError{Message: "failed to check host-wide TURN allocation", Cause: err}
	}
	if len(results) == 0 {
		return false, nil
	}
